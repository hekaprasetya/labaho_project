...
if ($form_qrcode_data) {
    ...
    $lokasi = $form_qrcode_data['lokasi'];
    $tbl_name = "utility_pompa";
    ...
    $isi_row = array("nama", "tanggal", "lokasi", "jokey_on", "jokey_off", "main_on", "main_off", "catatan");
    $header = array("Pemeriksa", "Tanggal Pengerjaan", "LOKASI", "JOKEY ON", "JOKEY OFF", "MIAN ON", "MAIN OFF", "CATATAN");


    // $tbl_name = isset($_GET['tbl_name']) ? "utility_" . htmlspecialchars($_GET['tbl_name'], ENT_QUOTES, 'UTF-8') : 'kosong';
    // $id_pompa = isset($_GET['id_pompa']) ? htmlspecialchars($_GET['id_pompa'], ENT_QUOTES, 'UTF-8') : 'kosong';
...
WHERE a.lokasi='$lokasi'";
    ...
    if ($cek == 0) {
        ...
        header("Location: ./form_qrcode_pompa.php");
        exit;
    }
    ...
    <body>
        <div class="row">
            <div class="disp">
                ...
                <h2 class="up">Master Utility POMPA</h2>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <?php foreach ($isi_row as $kolom) { ?>
                                            <td><?php if ($kolom != "tanggal") :
                                                    echo isset($row[$kolom]) ? $row[$kolom] : "<p class='red-text'>No Data</p>";
                                                else :
                                                    echo isset($row[$kolom]) ? indoDateTime($row[$kolom]) : "<p class='red-text'>No Data</p>";
                                                endif; ?></td>
                                        <?php } ?>
                                    </tr>
