...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if ($_REQUEST['dari_tanggal'] == "" || $_REQUEST['sampai_tanggal'] == "") {
            header("Location: ./admin.php?page=report_lpt");
            die();
        } else {

            $query = mysqli_query($config, "SELECT * FROM tbl_lpt 
                                                     JOIN tbl_surat_masuk 
                                                     ON tbl_lpt.id_surat = tbl_surat_masuk.id_surat
                                                     JOIN tbl_approve_lpt
                                                     ON tbl_lpt.id_lpt = tbl_approve_lpt.id_lpt 
                                                     JOIN tbl_verifikasi_lpt
                                                     ON tbl_lpt.id_lpt = tbl_verifikasi_lpt.id_lpt
                                                     WHERE tgl_lpt BETWEEN '$dari_tanggal' AND '$sampai_tanggal'");
                            ...
                            <div class="z-depth-1">
                                <nav class="secondary-nav">
                                    <div class="nav-wrapper blue-grey darken-1">
                                        <div class="col 12">
                                            <ul class="left">
                                                <li class="waves-effect waves-light"><a href="?page=report_lpt" class="judul"><i class="material-icons">print</i> Cetak Laporan E-LPT<a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </nav>
            ...
            echo '<img class="logodisp" src="./upload/' . $logo . '"/>';
             echo '<span></span><br/>';
            echo '<span><h6>LAPORAN E - LPT</h6></span><br/>';
                    ...
                    <div id="colres" class="warna cetak">
                        <table class="bordered" id="tbl" width="100%">
                            <thead class="blue lighten-4">
                                <tr>
                                    <th width="2%">No</th>
                                    <th width="10%">No.LPT<br/><hr/>Tgl.LPT</th>
                                    <th width="10%">No.PMK<br/><hr/>Tgl.PMK</th>
                                    <th width="5%">Nama Teknisi</th>
                                    <th width="18%">Lokasi Pengerjaan<br/><hr/> Jenis Pekerjaan</th>
                                    <th width="12%">Nama Perusahaan</th>
                                    <th width="15%">Peminta</th>
                                    <th width="10%">Verifikator</th>
                                     <th width="10%">Mng.Teknik</th>
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                while ($row = mysqli_fetch_array($query)) {
                    ...
                    echo '
                                 <tr>
                                        <td>' . $no . '</td>
                                        <td><strong>' . $row['no_lpt'] . '</strong><br/><hr/>' . indoDate($row['tgl_lpt']) . '</td>
                                        <td>' . $row['no_agenda'] . '<br/><hr/>' . indoDate($row['tgl_surat']) . '</td>
                                        <td>' . $row['nama_tk'] . '</td>
                                        <td>' . $row['lokasi_pengerjaan'] . '<br/><hr/>' . $row['jenis_pekerjaan'] . '</td>
                                        <td>' . $row['nama_perusahaan'] . '</td>
                                        <td>' . $row['peminta'] . '</td>
                                        <td><strong><i>' . $row['nama_verifikator'] . '</i></strong></td>
                                         <td>' . $row['ttd_kabag'] . '</td>
                                        ';
                        ...
                        <div class="z-depth-1">
                            <nav class="secondary-nav">
                                <div class="nav-wrapper blue-grey darken-1">
                                    <div class="col 12">
                                        <ul class="left">
                                            <li class="waves-effect waves-light"><a href="?page=ask" class="judul"><i class="material-icons">print</i> Cetak Laporan E-LPT<a></li>
                                        </ul>
                                    </div>
                                </div>
                            </nav>
