<head>
    ...
    if (empty($_SESSION['admin'])) {
        ...
        if (isset($_REQUEST['submita'])) {
            ...
            $cek_data_query = mysqli_query($config, "SELECT * FROM tbl_approve_utility WHERE id_genset='$id_genset'");
            $cek_data = mysqli_num_rows($cek_data_query);
            $cek_data_row = mysqli_fetch_array($cek_data_query);
            if ($cek_data == 0) {
                $query = mysqli_query($config, "INSERT INTO tbl_approve_utility(status_utility, id_genset, id_user)
                                                VALUES('$status_utility', '$id_genset', '$id_user')");
            } else {
                $query = mysqli_query($config, "UPDATE tbl_approve_utility SET status_utility='$status_utility',
                    id_genset='$id_genset', id_user='$id_user' WHERE id_app_utility=$cek_data_row[id_app_utility]");
            }
            if ($query == true) {
    ...
    ?><script language="javascript">
                ...
                $actArray = [
                    'add' => 'tambah_genset.php',
                    'edit' => 'edit_genset.php',
                    'del' => 'hapus_genset.php',
                    'laporan' => 'report_genset.php'
                ];
                if (isset($_REQUEST['act'])) {
                    ...
                    $query = mysqli_query($config, "SELECT utility_genset FROM tbl_sett");
                    list($utility_genset) = mysqli_fetch_array($query);

                    //pagging
                    $limit = $utility_genset;
                    $pg = @$_GET['pg'];
                    if (empty($pg)) {
                        $curr = 0;
                        $pg = 1;
                    ...
                    $genset->id_name = 'genset';

                    // nama tabel utama halaman
                    $genset->tbl_name = 'utility_genset';

                    // page halaman pemangilan di admin
                    $genset->pg_name = 'genset';

                    // judul pada secon nav
                    $genset->judul = "GENSET";

                    // icon untuk judul
                    $genset->icon_judul = "build";

                    // status approve 
                    $genset->status = "status_utility";
                ...
                ?>
                    ...
                    <br>
                    <button class="btn waves-effect waves-light light-blue" style="border-radius:7px;" onclick="window.location.href = '?page=<?= $genset->pg_name ?>&act=laporan'">Laporan
                    ...
                    <div class="row">
                        ...
                        $query_genset = "SELECT a.*,
                    b.id_app_utility,status_utility,waktu_utility,
                    c.nama
                    FROM utility_genset a
                    LEFT JOIN tbl_approve_utility b ON a.id_genset = b.id_genset 
                            ...
                            <div class="col s12" style="margin-top: -18px;">
                                <div class="card blue lighten-5">
                                    <div class="card-content">
                                        <p class="description">Hasil pencarian untuk kata kunci <strong>"<?= stripslashes($cari) ?>"</strong><span class="right"><a href="?page=genset"><i class="material-icons md-36" style="color: #333;">clear</i></a></span></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col m12" id="colres">
                                <table class="bordered centered" id="tbl">
                                    ...
                                    <tbody>
                                        ...
                                        $query_genset .= "WHERE nama_genset LIKE '%$cari%' 
                                    OR nama LIKE '%$cari%' 
                                    OR c_accu LIKE '%$cari%' 
                                    OR c_pelumas LIKE '%$cari%' 
                                    OR c_pendingin LIKE '%$cari%' 
                                    OR c_phg LIKE '%$cari%' 
                                    OR c_bb LIKE '%$cari%' 
                                    OR c_panel LIKE '%$cari%' 
                                    OR tgl_genset LIKE '%$cari%' 
                                    ORDER by id_genset DESC";
                                        // meenggunakan fuction query
                                        $result = mysqli_query($config, $query_genset);
                                        $cek = mysqli_num_rows($result);
                                        if ($cek) {
                                            $no = 1;
                                            while ($row = $result->fetch_assoc()) {
                                                $id = $row['id_genset'];
                                                $genset->id = $row['id_genset'];
                                                $genset->file = $row['file'];
                                                $genset->dir = "upload/genset/$genset->file";

                                        ?>
                                               <tr>
                                            <td><?= $no ?></td>
                                            <td><?= $row['no_form'] ?> <br>
                                                <hr><?= indoDateTime($row['tgl_genset']) ?>
                                            </td>
                                            <td><?= $row['nama'] ?><br>
                                                <hr><?= $row['nama_genset'] ?>
                                            </td>
                                                ...
                                                ?>
                                                    ...
                                                    ?>
                                                        <div class="row">
                                                            ...
                                                            <a class="btn small blue darken-1  waves-effect waves-light tooltipped" data-position="left" data-tooltip="Edit Alat" href="?page=<?= $genset->pg_name ?>&act=edit&id_<?= $genset->id_name ?>=<?= $id ?>">
                                                                <i class="material-icons">edit</i></a>
                                                            <a class="btn small deep-orange waves-effect waves-light tooltipped" data-position="left" data-tooltip="Hapus Alat" href="?page=<?= $genset->pg_name ?>&act=del&id_<?= $genset->id_name ?>=<?= $id ?>">
                    ...
                    <div class="col m12" id="colres">
                        <table class="bordered centered" id="tbl">
                            ...
                            <tbody>
                                ...
                                // menambahkan untuk variabel query untuk mencari
                                $query_genset .= " ORDER BY id_genset DESC LIMIT $curr, $limit";
                                // meenggunakan fuction query
                                $result = mysqli_query($config, $query_genset);
                                $cek = mysqli_num_rows($result);
                                if ($cek) {
