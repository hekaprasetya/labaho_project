...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        $query = mysqli_query($config, "SELECT * FROM tbl_approve_op_kabag WHERE id_op='$id_op'");
        $no = 1;
        list($id_op) = mysqli_fetch_array($query);
        {

            $id_op = $_REQUEST['id_op'];
            $status_op_kabag = $_POST['status_op_kabag'];
            $id_user = $_SESSION['id_user'];
            $cek_data_qry = mysqli_query($config, "select * from tbl_approve_op_kabag where id_op='$id_op'");
            $cek_data = mysqli_num_rows($cek_data_qry);
            $cek_data_row = mysqli_fetch_array($cek_data_qry);
            if ($cek_data == 0) {
                $query = mysqli_query($config, "INSERT INTO tbl_approve_op_kabag(status_op_kabag,id_op,id_user)
                                        VALUES('$status_op_kabag','$id_op','$id_user')");
            } else {
                 $query = mysqli_query($config, "UPDATE tbl_approve_op_kabag SET
                status_op_kabag='$status_op_kabag',
                id_op='$id_op',id_user='$id_user' WHERE id_app_op_kabag=$cek_data_row[id_app_op_kabag]");
            }
                ...
                <div class="row jarak-form">
                    <form class="col s12" method="post" action="">
                        <div class="input-field col s9">
                            ...
                            <select name="status_op_kabag" class="materialize-textarea validate" id="status_op_kabag" required>
                                <option value=""></option>
                        ...
                        <!--div class="col 6">
                            <button type="reset" onclick= window.location.href="./admin.php?page=pp&act=app_kabag_pp&id_pp=' . $row['id_barang_detail'] . '"; class="btn small deep-orange waves-effect waves-light">BATAL <i class="material-icons">clear</i></button>
                        </div-->
                    </form>
                </div>
            </div>
