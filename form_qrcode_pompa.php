...
if (isset($_REQUEST['submitQR'])) {
    $lokasi = htmlspecialchars($_POST['kode'], ENT_QUOTES, 'UTF-8');
    $csrf_token = htmlspecialchars($_POST['x'], ENT_QUOTES, 'UTF-8');

    // Menentukan nilai yang diizinkan
    $allowedValues = array("GP-UTAMA", "GP-EXTENSION");

    // Validasi
    if (in_array($lokasi, $allowedValues)) {
        // Lakukan apa yang perlu dilakukan jika nilai valid
        echo "Lokasi yang dipilih: " . $lokasi;
    } else {
        // Tindakan jika nilai tidak valid
        echo "Pilih lokasi yang valid.";
    }
    ...
    $_SESSION['form_qrcode'] = ['lokasi' => $lokasi];
    // Hapus token CSRF setelah digunakan (opsional)
    unset($_SESSION['csrf_token']);
    // Meneruskan nilai ke halaman qrcode_pompa.php tanpa menyertakan data di URL
    header("Location: ./qrcode_pompa.php");
    exit;
}
