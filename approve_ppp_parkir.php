...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submita'])) {
        ...
        $query = mysqli_query($config, "SELECT * FROM tbl_approve_ppp_parkir WHERE id_ppp='$id_ppp'");
        $no = 1;
        list($id_ppp) = mysqli_fetch_array($query); {

            $status_ppp_parkir = $_POST['status_ppp_parkir'];
            $id_ppp = $_REQUEST['id_ppp'];
            $id_user = $_SESSION['id_user'];
            $cek_data_qry = mysqli_query($config, "select * from tbl_approve_ppp_parkir where id_ppp='$id_ppp'");
            $cek_data = mysqli_num_rows($cek_data_qry);
            $cek_data_row = mysqli_fetch_array($cek_data_qry);
            if ($cek_data == 0) {
                $query = mysqli_query($config, "INSERT INTO tbl_approve_ppp_parkir(status_ppp_parkir,id_ppp,id_user)
                                            VALUES('$status_ppp_parkir','$id_ppp','$id_user')");
            } else {
                $query = mysqli_query($config, "UPDATE tbl_approve_ppp_parkir SET
                    status_ppp_parkir='$status_ppp_parkir',
                    id_ppp='$id_ppp',
        ...
        if (isset($_POST['submit_cari'])) {
            ...
            list($id_ppp) = mysqli_fetch_array($query); {
                ...
                if ($query == true) {
                    $_SESSION['succAdd'] = 'SUKSES! Data berhasil ditambahkan';
                    echo '<script language="javascript">
                                                window.location.href="./admin.php?page=ppp&act=app_ppp_parkir";
                                              </script>';
                } else {
                    $_SESSION['errQ'] = 'ERROR! Ada masalah dengan query';
                    echo '<script language="javascript">window.history.back();</script>';
                                ...
                                <div class="col m12">
                                    <ul class="left">
                                        ...
                                        <div class="col s6 show-on-medium-and-up">
                                            <form method="post" action="?page=ppp&act=app_ppp_parkir">
            ...
            if (isset($_REQUEST['submito'])) {
                ...
                echo '
                            <div class="col s12" style="margin-top: -18px;">
                                <div class="card blue lighten-5">
                                    ...
                                    <p class="description">Hasil pencarian untuk kata kunci <strong>"' . stripslashes($cari) . '"</strong><span class="right"><a href="?page=ppp&act=app_ppp_parkir"><i class="material-icons md-36" style="color: #333;">clear</i></a></span></p>
                                    </div>
                                </div>
                            </div>
                    ...
                    while ($row = mysqli_fetch_array($query)) {
                        ...
                        if (!empty($row['status_ppp_parkir'])) {
                            ?> <strong><?= $row['status_ppp_parkir'] ?></a></strong>
                                ...
                                <td><a class="btn small purple darken-2 waves-effect waves-light tooltipped modal-trigger" data-position="left" data-tooltip="Approval" href="#modal<?= $no ?>">
                                            ...
                                            <div class="row jarak-form">
                                                <form class="col s12" method="post" action="">
                                                    <div class="input-field col s7">
                                                        ...
                                                        <select name="status_ppp_parkir" class="browser-default validate" id="status_ppp_parkir" required>
                                                            <option value=""></option>
                ...
                if (isset($_REQUEST['simpan'])) {
                    ...
                    if ($query == true) {
                        header("Location: ./admin.php?page=ppp&act=app_ppp_parkir");
                        die();
                    }
                }
                ?>
                ...
                if ($cdata > $limit) {
                    ...
                    if ($pg > 1) {
                        $prev = $pg - 1;
                        echo '<li><a href="?page=ppp&act=app_ppp_parkir&pg=1"><i class="material-icons md-48">first_page</i></a></li>
                                      <li><a href="?page=ppp&act=app_ppp_parkir&pg=' . $prev . '"><i class="material-icons md-48">chevron_left</i></a></li>';
                    } else {
                    ...
                    for ($i = 1; $i <= $cpg; $i++) {
                        if ((($i >= $pg - 3) && ($i <= $pg + 3)) || ($i == 1) || ($i == $cpg)) {
                            if ($i == $pg)
                                echo '<li class="active waves-effect waves-dark"><a href="?page=ppp&act=app_ppp_parkir&pg=' . $i . '"> ' . $i . ' </a></li>';
                            else
                                echo '<li class="waves-effect waves-dark"><a href="?page=ppp&act=app_ppp_parkir&pg=' . $i . '"> ' . $i . ' </a></li>';
                        }
                    }
                    ...
                    if ($pg < $cpg) {
                        $next = $pg + 1;
                        echo '<li><a href="?page=ppp&act=app_ppp_parkir&pg=' . $next . '"><i class="material-icons md-48">chevron_right</i></a></li>
                                      <li><a href="?page=ppp&act=app_ppp_parkir&pg=' . $cpg . '"><i class="material-icons md-48">last_page</i></a></li>';
                    } else {
