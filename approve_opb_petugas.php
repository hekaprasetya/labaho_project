...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submita'])) {
        ...
        $query = mysqli_query($config, "SELECT * FROM tbl_approve_opb_petugas WHERE id_opb='$id_opb'");
        $no = 1;
        list($id_opb) = mysqli_fetch_array($query); {

            $id_opb = $_REQUEST['id_opb'];
            $status_opb_petugas = $_POST['status_opb_petugas'];
            $id_user = $_SESSION['id_user'];
            $cek_data_qry = mysqli_query($config, "select * FROM tbl_approve_opb_petugas where id_opb='$id_opb'");
            $cek_data = mysqli_num_rows($cek_data_qry);
            $cek_data_row = mysqli_fetch_array($cek_data_qry);
            if ($cek_data == 0) {
                $query = mysqli_query($config, "INSERT INTO tbl_approve_opb_petugas(status_opb_petugas,id_opb,id_user)
                                        VALUES('$status_opb_petugas','$id_opb','$id_user')");
            } else {
                $query = mysqli_query($config, "UPDATE tbl_approve_opb_petugas SET
                status_opb_petugas='$status_opb_petugas',
                id_opb='$id_opb',id_user='$id_user' WHERE id_app_opb_petugas=$cek_data_row[id_app_opb_petugas]");
            }
        ...
        if (isset($_REQUEST['submit'])) {
            //print_r($_POST);die;
            $id_opb = $_REQUEST['id_opb'];
            $query = mysqli_query($config, "SELECT * FROM tbl_petugas_opb WHERE id_opb='$id_opb'");
            $no = 1;
            list($id_opb_detail) = mysqli_fetch_array($query); {
                $id_opb = $_REQUEST['id_opb'];
                $terbit_petugas = $_POST['terbit_petugas'];
                $id_user = $_SESSION['id_user'];
                $cek_data_qry = mysqli_query($config, "select * from tbl_petugas_opb where id_opb_detail='$id_opb_detail'");
                $cek_data = mysqli_num_rows($cek_data_qry);
                $cek_data_row = mysqli_fetch_array($cek_data_qry);
                if ($cek_data == 0) {
                    $query = mysqli_query($config, "INSERT INTO tbl_petugas_opb(status_opb_petugas,id_opb_detail,id_opb,id_user)
                                        VALUES('$status_opb_petugas''$id_opb_detail','$id_opb','$id_user')");
                    ...
                    $query = mysqli_query($config, "UPDATE tbl_petugas_opb SET
                status_opb_petugas='$status_opb_petugas',
                ...
                if ($query == true) {
                    $_SESSION['succAdd'] = 'SUKSES! Data berhasil ditambahkan';
                    echo '<script language="javascript">
                                                window.location.href="./admin.php?page=opb&act=app_petugas_opb&id_opb=' . $id_opb . '";
                                              </script>';
                } else {
                    $_SESSION['errQ'] = 'ERROR! Ada masalah dengan query';
                    echo '<script language="javascript">window.history.back();</script>';
                            ...
                            <div class="row jarak-form">
                                <form class="col s12" method="post" action="">
                                    <div class="input-field col s9">
                                        ...
                                        <select name="status_opb_petugas" class="browser-default validate" id="status_opb_petugas" required>
                                            <option value=""></option>
