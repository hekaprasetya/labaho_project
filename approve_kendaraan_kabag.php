...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submita'])) {
        ...
        $status_kendaraan_kabag = $_REQUEST['status_kendaraan_kabag'];
        $id_user = $_SESSION['id_user'];

        // Periksa apakah data sudah ada dalam tabel tbl_approve_kendaraan_kabag
        if ($status_kendaraan_kabag === 'Diterima') {
            // Perbarui status_kendaraan_kabag di tbl_approve_kendaraan_kabag
            $cek_data_query = mysqli_query($config, "SELECT * FROM tbl_approve_kendaraan_kabag WHERE id_pinjam_kendaraan='$id_pinjam_kendaraan'");
            $cek_data = mysqli_num_rows($cek_data_query);
            $cek_data_row = mysqli_fetch_array($cek_data_query);

            if ($cek_data == 0) {
                $query = mysqli_query($config, "INSERT INTO tbl_approve_kendaraan_kabag(status_kendaraan_kabag, id_pinjam_kendaraan, id_user)
                                            VALUES('$status_kendaraan_kabag', '$id_pinjam_kendaraan', '$id_user')");
            } else {
                $query = mysqli_query($config, "UPDATE tbl_approve_kendaraan_kabag SET
                    status_kendaraan_kabag='$status_kendaraan_kabag', id_pinjam_kendaraan='$id_pinjam_kendaraan', id_user='$id_user' WHERE id_app_kendaraan_kabag=$cek_data_row[id_app_kendaraan_kabag]");
            }
        } else {
        ...
        if (isset($_REQUEST['submit'])) {
            $id_pinjam_kendaraan = $_REQUEST['id_pinjam_kendaraan'];
            $query = mysqli_query($config, "SELECT * FROM tbl_approve_kendaraan_kabag WHERE id_pinjam_kendaraan='$id_pinjam_kendaraan'");
            $no = 1;
            list($id_pinjam_kendaraan) = mysqli_fetch_array($query); {
                $id_pinjam_kendaraan = $_REQUEST['id_pinjam_kendaraan'];
                $status_kendaraan_kabag = $_POST['status_kendaraan_kabag']; // Anda perlu mengganti 'status_opb_kabag' dengan nama yang sesuai dari formulir
                $id_user = $_SESSION['id_user'];

                $cek_data_query = mysqli_query($config, "SELECT * FROM tbl_approve_kendaraan_kabag WHERE id_pinjam_kendaraan='$id_pinjam_kendaraan'");
                $cek_data = mysqli_num_rows($cek_data_query);
                $cek_data_row = mysqli_fetch_assoc($cek_data_query);
                if ($cek_data == 0) {
                    $query = mysqli_query($config, "INSERT INTO tbl_approve_kendaraan_kabag(status_kendaraan_kabag, id_pinjam_kendaraan, id_user)
            VALUES('$status_kendaraan_kabag', '$id_pinjam_kendaraan', '$id_user')");
                } else {
            ...
            // Kode berikut ini akan dijalankan jika submit tidak tersedia
            ...
            // Tambahkan kode Anda yang sesuai di sini
                ...
                <div class="col m12" id="colres">
                    <table class="bordered  centered highlight" id="tbl">
                        ...
                        <tbody>
                            ...
                            b.id_app_kendaraan_kabag,status_kendaraan_kabag,
                            c.nama 
                            ...
                            <div class="row jarak-form">
                                <form class="col s12" method="post" action="">
                                    <div class="input-field col s9">
                                        ...
                                        <select name="status_kendaraan_kabag" class="browser-default validate" id="status_kendaraan_kabag" required>
