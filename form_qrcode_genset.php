...
if (isset($_REQUEST['submitQR'])) {
    $id_utility = htmlspecialchars($_POST['kode'], ENT_QUOTES, 'UTF-8');
    $csrf_token = htmlspecialchars($_POST['x'], ENT_QUOTES, 'UTF-8');

    if (!preg_match("/^[0-9]+$/", $id_utility)) {
        // Kode barang tidak valid
        echo "Kode barang harus berupa angka.";
        exit;
    }
    ...
    $_SESSION['form_qrcode'] = ['id_utility' => $id_utility];
    // Hapus token CSRF setelah digunakan (opsional)
    unset($_SESSION['csrf_token']);
    // Meneruskan nilai ke halaman qrcode_panel.php tanpa menyertakan data di URL
    header("Location: ./qrcode_genset.php");
    exit;
}
        ...
        <div class="form-container">
            ...
            <form class="form" method="post">
                <div class="form-group">
                    ...
                    <input type="number" id="kode" name="kode" placeholder=" Masukan Kode Item" pattern="[0-9]+" title="Kode barang harus berupa angka" required>
                </div>
