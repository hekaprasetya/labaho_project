    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if( $_REQUEST['ttd_kabag'] == "" ){
                ...
                //$ttd_spv    = $_REQUEST['ttd_spv'];
                $id_user    = $_SESSION['id_user'];

                //validasi input data
                if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $ttd_kabag)){
                    $_SESSION['ttd_kabag'] = 'Form TTd Kabag hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,) minus(-). kurung() dan garis miring(/)';
                    echo '<script language="javascript">window.history.back();</script>';
                } else {

                    //if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $ttd_spv)){
                    //    $_SESSION['ttd_spv'] = 'Form TTD Spv hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan(&), underscore(_), kurung(), persen(%) dan at(@)';
                      //  echo '<script language="javascript">window.history.back();</script>';
            ...
            <div class="row jarak-form">
                ...
                <form class="col s12" method="post" action="">
                    ...
                    <div class="row">
                     ...
                     <!--div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">low_priority</i><label>Supervisor</label><br/>
                            <div class="input-field col s11 right">
                                <select class="browser-default validate" name="ttd_spv" id="ttd_spv" required>
                            ...
                            <?php
                                if(isset($_SESSION['ttd_spv'])){
                                    $ttd_spv = $_SESSION['ttd_spv'];
                                    echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$ttd_spv.'</div>';
                                    unset($_SESSION['ttd_spv']);
                                }
                            ?>
                        </div-->
                    
                        <div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">low_priority</i><label>Manager/Asman</label><br/>
                            <div class="input-field col s11 right">
